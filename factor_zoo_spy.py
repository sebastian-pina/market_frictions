...
def spy_daily_ret(date_str):
    spy_key = f'AAPL_{date_str}'
    try:
        if spy_key not in _FEAT_CACHE:
            path = DATA_DIR / 'AAPL' / 'mbp-10' / f'{date_str}.parquet'
...
def plot_cumulative_pnl(results, filename='factor_zoo_spy_pnl.png'):
    """
...
def plot_rolling_sharpe(results, window=5, filename='factor_zoo_spy_rolling_sharpe.png'):
...
def plot_importance(ols_model, ols_scaler, lgbm_model, ablation_drops,
                    filename='factor_zoo_spy_importance.png'):
...
def plot_ic_comparison(ic_table, filename='factor_zoo_spy_ic_comparison.png'):
...
def plot_inventory(daily_df_ols, daily_df_lgbm, filename='factor_zoo_spy_inventory.png'):
...
def save_summary_table(metrics_table, ic_table, filename='factor_zoo_spy_metrics.csv'):
    rows = []
...
if __name__ == '__main__':
    ...
    print(f'Metrics saved to factor_zoo_spy_metrics.csv')
