...
**Two-track filter:**
- *SEP days* (quarterly, dot-plot released): trade only when $|z_t| = |\Delta\text{DGS2} / \sigma_{30d}| \leq 1.0$. Large DGS2 moves signal genuine forward-guidance surprises that overwhelm the mechanical reversion.
- *Non-SEP days* (inter-meeting, rate decision only): trade unconditionally. Without a dot-plot, only the rate decision is new information — a quantity that is almost always well-priced before 2:00 PM. The mechanical overshoot reliably dominates.
...
The FOMC contrarian strategy is viable in hold/cut monetary policy regimes (Sharpe 2.08, p = 0.046, ex-2022 ex-Dec-18) and fails exclusively in the hiking cycle. The two-track filter operationalizes the key distinction: non-SEP meetings lack forward-guidance content and the mechanical overshoot dominates unconditionally (22 events, Sharpe 3.95, p = 0.001, 77\% hit rate); SEP meetings require the DGS2 z-score screen because the dot-plot can deliver a path surprise that overwhelms the reversion (7 small-z SEP events: +\$388; 11 medium/large-z SEP events: $-\$2,046$). Combined, the two-track strategy trades 29 of 40 events and generates +\$3,614 total at Sharpe 2.51 (p = 0.018, 69\% hit rate) — statistically significant at the 5\% level.

**For a practitioner:** implement the two-track filter — trade all non-SEP meetings and apply the DGS2 $|z| \leq 1.0$ screen only on SEP days. In a hold/cut environment, expect approximately 6–7 traded events per year (all 4–5 non-SEP meetings plus 1–2 small-z SEP meetings). Avoid active hiking cycles. Capacity ceiling remains near \$200,000 notional per trade. The expected annual Sharpe in a hold/cut environment is approximately 2.0–3.5 net of TC.
