...
print('Loading NVDA train...')
nvda1 = load_bars('NVDA', TRAIN_DATES)

common = spy1.index.intersection(nvda1.index)
spy1   = spy1.loc[common]
nvda1  = nvda1.loc[common]
...
print(f'NVDA 1s bars: {len(nvda1):,}')
print(f'OFI SPY  range: [{spy1["ofi_mean"].min():.3f}, {spy1["ofi_mean"].max():.3f}]')
print(f'Spread NVDA mean: ${nvda1["spread_mean"].mean():.4f}')
print(f'NVDA half-spread (bps): {nvda1["spread_mean"].mean() / nvda1["mid_close"].mean() * 1e4 / 2:.3f} bps per side')
spy1.tail(3)
...
nvda_ret = nvda1['mid_close'].pct_change().fillna(0) * 1e4
spy_ofi  = spy1['ofi_mean'].fillna(0)

lags = range(-5, 16)   # -5s to +15s
xcorr_ret = [spy_ret.corr(nvda_ret.shift(-lag))  for lag in lags]
xcorr_ofi = [spy_ofi.corr(nvda_ret.shift(-lag))  for lag in lags]
...
axes[0].set_xlabel('Lag (seconds) — positive = SPY leads NVDA')
axes[0].set_ylabel('Cross-correlation')
axes[0].set_title('SPY return -> NVDA return (1s bars)')
...
axes[1].set_title('SPY OFI -> NVDA return (1s bars)')

plt.tight_layout()
plt.savefig('figures/xcorr_spy_nvda.png', dpi=150)
plt.show()

print('Lag(s)  SPY_ret->NVDA_ret   SPY_OFI->NVDA_ret')
for i, lag in enumerate(lags):
    print(f'  {lag:+3d}s   {xcorr_ret[i]:+.5f}              {xcorr_ofi[i]:+.5f}')

best_lag = list(lags)[int(np.argmax(xcorr_ret[5:], 0)) + 5]   # best positive lag
print(f'\nBest positive lag for price lead: +{best_lag}s  (corr={spy_ret.corr(nvda_ret.shift(-best_lag)):.5f})')
...
# Signal uses SPY 1s price return (positive correlation with NVDA +1s)
...
SPREAD_MAX   = 0.50    # NVDA max spread in $ — NVDA mean spread ~$0.16, use $0.50 cap


def build_signal(spy1: pd.DataFrame, nvda1: pd.DataFrame) -> pd.Series:
    """
    Signal = +1 (buy NVDA) when:
      - SPY 1s return > RET_THRESH bps  (SPY mid moved up sharply)
      - SPY OFI > OFI_THRESH            (buy-side book pressure confirms direction)
      - NVDA spread < SPREAD_MAX        (can execute at reasonable cost)

    Signal = -1 (sell NVDA) when mirror conditions hold.

    Entry at next bar (1s delay), hold HOLD_BARS bars = 5 seconds.
    """
    spy_ret1 = spy1['mid_close'].pct_change().fillna(0) * 1e4   # 1s return
    nvda_spread = nvda1['spread_mean']

    long_entry  = (spy_ret1 >  RET_THRESH) & (spy1['ofi_mean'] >  OFI_THRESH) & (nvda_spread < SPREAD_MAX)
    short_entry = (spy_ret1 < -RET_THRESH) & (spy1['ofi_mean'] < -OFI_THRESH) & (nvda_spread < SPREAD_MAX)
...
train_signal = build_signal(spy1, nvda1)
...
# === cell 5 ===
def backtest(signal: pd.Series, nvda1: pd.DataFrame, hold_bars: int = HOLD_BARS,
    ...
    """
    prices  = nvda1['mid_close'].values
    spreads = nvda1['spread_mean'].values
    times   = nvda1.index
    dates   = nvda1['date'].values        # normalized date for boundary check
    sig     = signal.reindex(nvda1.index, fill_value=0).values
...
trades_train = backtest(train_signal, nvda1)
...
plot_cumulative_pnl(trades_train, 'Lead-Lag SPY→NVDA — Training Set', 'leadlag_train_pnl.png')

# === cell 8 ===
print('Loading test data...')
spy1_test  = load_bars('SPY',  TEST_DATES)
nvda1_test = load_bars('NVDA', TEST_DATES)

common_test = spy1_test.index.intersection(nvda1_test.index)
spy1_test   = spy1_test.loc[common_test]
nvda1_test  = nvda1_test.loc[common_test]

print(f'Test 1s bars: {len(common_test):,}')

test_signal  = build_signal(spy1_test, nvda1_test)
trades_test  = backtest(test_signal, nvda1_test)

print('\n=== TEST SET ===')
m_test = compute_metrics(trades_test, label='test')

plot_cumulative_pnl(trades_test, 'Lead-Lag SPY->NVDA -- Test Set (OOS)', 'leadlag_test_pnl.png')
