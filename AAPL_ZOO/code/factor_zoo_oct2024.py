"""
Factor Zoo – AAPL L2 Microstructure (12 book features)
=======================================================
Train: Jan-Feb 2024 (18 days)
Val:   Mar-Apr 2024 (7 days)   [within-sample OOS check]
OOS:   Oct 2024    (15 days)   [official test — Databento ITCH MBP-10]
...
def compute_features(mbp):
    ...
    # 11. Spread in bps (stationary: 0.01/190*1e4 ≈ 0.53 bps regardless of price era)
...
def build_dataset(dates, tag='building'):
    ...
    for d in dates:
        try:
            if d not in _FEAT_CACHE:
                mbp = load_day(d)
                _FEAT_CACHE[d] = compute_features(mbp)
                del mbp
            feat = _FEAT_CACHE[d]
...
# ── Ablation (drop-one IC on training set) ────────────────────────────────────
...
# ── SPY daily return (for beta calculation) ───────────────────────────────────
...
def plot_cumulative_pnl(results, filename='factor_zoo_oct_pnl.png'):
    """
    ...
    fig.suptitle('Cumulative PnL — AAPL Microstructure Strategy (60s hold)', fontsize=12, y=1.01)
...
def plot_rolling_sharpe(results, window=5, filename='factor_zoo_oct_rolling_sharpe.png'):
...
def plot_importance(ols_model, ols_scaler, lgbm_model, ablation_drops,
                    filename='factor_zoo_oct_importance.png'):
    ...
    plt.suptitle('Feature Importance — AAPL Microstructure Factor Zoo', fontsize=12)
    plt.tight_layout()
    fig.savefig(FIG_DIR / filename, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'  Saved {filename}')
...
def plot_ic_comparison(ic_table, filename='factor_zoo_oct_ic_comparison.png'):
    """Bar chart: IC by model × period."""
...
def plot_inventory(daily_df_ols, daily_df_lgbm, filename='factor_zoo_oct_inventory.png'):
...
# ── Summary table ─────────────────────────────────────────────────────────────
def save_summary_table(metrics_table, ic_table, filename='factor_zoo_metrics.csv'):
    rows = []
...
if __name__ == '__main__':
    ...
    # ── 1. Build datasets ────────────────────────────────────────────────────
    print('=' * 60)
    print('FACTOR ZOO — AAPL MBP-10 (11 Book Features)')
    print('=' * 60)
    ...
    # ── 5. Threshold optimization on val set (OLS) ───────────────────────────
    ...
    print(f'Metrics saved to factor_zoo_metrics.csv')
