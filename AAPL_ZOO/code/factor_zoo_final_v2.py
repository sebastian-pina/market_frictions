"""
Factor Zoo — Microstructure L2 Strategy  (ProjectPlan-compliant, final)
=========================================================================
...
11 features from MBP-10 only (trade-schema features 13-15 dropped —
...
ProjectPlan compliance
  ✓ Execution : BUY fills at ask_px_00, SELL at bid_px_00; walk book if
                order exceeds top-of-book depth
  ✓ Stop-loss : early exit when unrealized loss > 1.5× spread per share
  ✓ Signal flip: early exit if model sign reverses past threshold
  ✓ Hit rate  : % of individual TRADES with positive net PnL
  ✓ Rolling Sharpe: 21-day window
  ✓ Label winsorization at 1st / 99th pct of training labels
  ✓ Inventory  : position in shares reconstructed from trade log
  ✓ Capacity   : $10M / $100M / $1B (Almgren-Chriss impact model)
  ✓ All 5 required plots + summary metrics CSV
"""
...
SL_MULT   = 1.5       # stop-loss multiplier (× spread per share)
...
_SPY_CACHE:  dict = {}   # key = date_str → daily return
...
def _walk_fill(bid_px_arr, ask_px_arr, bid_sz_arr, ask_sz_arr, i, direction, shares):
    ...
    direction=+1 → buy (walk ask levels), direction=-1 → sell (walk bid levels).
...
def backtest(dates, model, scaler, threshold, model_type, tag=''):
    ...
    Execution:
      • Entry: walk-book fill at ask (long) or bid (short)
      • Exit: 60s later at bid (long) or ask (short)
      • Early exits: stop-loss (loss > 1.5× spread/share) or signal flip

    PnL:
      gross = direction × (mid_exit − mid_entry) × shares
      TC    = |fill_entry − mid_entry| + |fill_exit − mid_exit|   (per share × shares)
      net   = gross − TC
    """
    trades    = []
    daily_pnl = {}
        ...
        try:
            ...
            for i in range(nb):
                ...
                for j in range(i + 1, min(exit_idx + 1, nb)):
                    ...
                    # entry_fill is already above micro[i] by half-spread — comparing
...
def compute_metrics(daily_df, trades_df, dates):
    ...
    avg_hold = float(trades_df['hold_s'].mean()) if (n_tr > 0 and 'hold_s' in trades_df.columns) else HOLD_S
...
def capacity_analysis(trades_df):
    ...
    Almgren-Chriss impact model:
      temp_impact  = η × σ × (Q / V) × √(T/τ)
      perm_impact  = γ × σ × (Q / V)

    where:
      σ = daily volatility of stock (approx from realized_vol)
      V = daily volume ($)
      Q = strategy notional per trade × n_trades_per_day
      η ≈ 0.142  (Almgren et al. 2005, AAPL proxy)
      γ ≈ 0.071  (permanent impact coefficient)
      T = 1 day, τ = 1 trade per signal
    ...
    T   = 1.0       # 1 trading day
    tau = 1.0 / 23  # 23 trades per day ≈ typical strategy frequency
...
def plot_cumulative_pnl(results, sym):
    ...
    fig.suptitle(f'Cumulative PnL — {sym} Microstructure Strategy (60s hold)',
                 fontsize=12, y=1.01)
...
def plot_rolling_sharpe(results, sym):
    ...
    ax.set_title(f'{ROLL_WIN}-day Rolling Sharpe (annualised) — {sym}', fontsize=11)
...
# ── Plot 3: Inventory (position in shares) ────────────────────────────────────
...
def plot_inventory(results, sym):
    ...
    axes[0].set_title(f'Intraday Position in Shares — {sym} (5-min resolution)', fontsize=11)
...
def plot_importance(ols_model, lgbm_model, ablation_drops, sym):
    ...
    plt.suptitle(f'Feature Importance — {sym} Microstructure Factor Zoo', fontsize=12)
    plt.tight_layout()
...
# ── Plot 5: Train vs Test IC comparison ───────────────────────────────────────
...
def plot_ic_comparison(ic_table, sym, break_even_ic=None):
    ...
    ax.set_title(f'IC by Model and Period — {sym}\n'
                 f'(Break-even IC = spread/σy; OOS IC must exceed this to profit)',
                 fontsize=10)
    ax.legend()
...
if __name__ == '__main__':
    print('=' * 65)
    print(f'FACTOR ZOO — {SYMBOL}  MBP-10  |  11 book features  |  60s hold')
    print('=' * 65)
    ...
    # ── 3. Train LightGBM (early-stop on val) ────────────────────────
    print(f'\n=== LightGBM [{SYMBOL}] ===')
